...
#!/usr/bin/env bash
set -euo pipefail
if [[ "${1:-}" == "--expect-version" && "${2:-}" == "0.16.0" && "${3:-}" == "version" ]]; then
    echo "seen-pkg 0.16.0"
    exit 0
fi
...
TARBALL="$DIST_DIR/seen-0.16.0-linux-x64.tar.gz"
test -f "$TARBALL"
RUNTIME_COMPONENT="$DIST_DIR/seen-runtime-0.16.0-linux-x64.tar.gz"
...
grep -Fqx \
    'seen-0.16.0-linux-x64/lib/seen/runtime/cuda/src/seen_cuda.cu' \
    "$TMP_DIR/installer-archive-contents.txt"
grep -Fqx \
    'seen_runtime/cuda/src/seen_cuda.cu' \
    "$TMP_DIR/runtime-component-contents.txt"
tar -xOf "$TARBALL" \
    seen-0.16.0-linux-x64/bin/compatibility-manifest.json \
...
GO_V4_PACKAGE="$GO_V4_DIR/seen-0.16.0-linux-x64"
...
GO_DUPLICATE_PACKAGE="$GO_DUPLICATE_DIR/seen-0.16.0-linux-x64"
...
MISMATCH_PACKAGE="$MISMATCH_DIR/seen-0.16.0-linux-x64"
...
if ! grep -Fq "Compiler version mismatch: release metadata expects 'Seen 0.16.0', got 'Seen 9.9.9'" \
...
PACKAGE_DIR="$EXTRACT_DIR/seen-0.16.0-linux-x64"
...
if ! "$PREFIX/bin/seen" --version | grep -q '0.16.0'; then
...
grep -Fq '## [0.16.0] - 2026-08-26' "$PACKAGE_DIR/share/doc/seen/CHANGELOG.md"
