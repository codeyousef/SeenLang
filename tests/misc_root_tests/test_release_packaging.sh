...
#!/usr/bin/env bash
set -euo pipefail
if [[ "${1:-}" == "--expect-version" && "${2:-}" == "0.22.4" && "${3:-}" == "version" ]]; then
    echo "seen-pkg 0.22.4"
    exit 0
fi
...
TARBALL="$DIST_DIR/seen-0.22.4-linux-x64.tar.gz"
test -f "$TARBALL"
COMPILER_COMPONENT="$DIST_DIR/seen-compiler-0.22.4-linux-x64"
test -x "$COMPILER_COMPONENT"
RUNTIME_COMPONENT="$DIST_DIR/seen-runtime-0.22.4-linux-x64.tar.gz"
...
grep -Fqx \
    'seen-0.22.4-linux-x64/lib/seen/runtime/cuda/src/seen_cuda.cu' \
    "$TMP_DIR/installer-archive-contents.txt"
grep -Fqx \
    'seen_runtime/cuda/src/seen_cuda.cu' \
    "$TMP_DIR/runtime-component-contents.txt"
tar -xOf "$TARBALL" \
    seen-0.22.4-linux-x64/bin/compatibility-manifest.json \
...
GO_V4_PACKAGE="$GO_V4_DIR/seen-0.22.4-linux-x64"
...
GO_DUPLICATE_PACKAGE="$GO_DUPLICATE_DIR/seen-0.22.4-linux-x64"
...
MISMATCH_PACKAGE="$MISMATCH_DIR/seen-0.22.4-linux-x64"
...
if ! grep -Fq "Compiler version mismatch: release metadata expects 'Seen 0.22.4', got 'Seen 9.9.9'" \
...
PACKAGE_DIR="$EXTRACT_DIR/seen-0.22.4-linux-x64"
...
"$PACKAGE_DIR/lib/seen/toolchain/verify-compiler-provenance.sh" \
    ...
    "$PACKAGE_DIR/bin/seen" 0.22.4 >/dev/null
...
if "$PACKAGE_DIR/lib/seen/toolchain/verify-compiler-provenance.sh" \
    "$PACKAGE_DIR/share/seen/compiler-provenance.env" \
    "$TAMPERED_COMPILER" 0.22.4 >/dev/null 2>&1; then
...
if ! "$PREFIX/bin/seen" --version | grep -q '0.22.4'; then
...
"$PREFIX/lib/seen/toolchain/verify-compiler-provenance.sh" \
    ...
    "$PREFIX/bin/seen" 0.22.4 >/dev/null
...
grep -Fq '## [0.22.4] - 2026-09-16' "$PACKAGE_DIR/share/doc/seen/CHANGELOG.md"
