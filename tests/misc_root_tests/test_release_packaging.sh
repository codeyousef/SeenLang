...
#!/usr/bin/env bash
set -euo pipefail
if [[ "${1:-}" == "--expect-version" && "${2:-}" == "0.14.0" && "${3:-}" == "version" ]]; then
    echo "seen-pkg 0.14.0"
    exit 0
fi
...
TARBALL="$DIST_DIR/seen-0.14.0-linux-x64.tar.gz"
test -f "$TARBALL"
tar -xOf "$TARBALL" \
    seen-0.14.0-linux-x64/bin/compatibility-manifest.json \
...
GO_V4_PACKAGE="$GO_V4_DIR/seen-0.14.0-linux-x64"
...
GO_DUPLICATE_PACKAGE="$GO_DUPLICATE_DIR/seen-0.14.0-linux-x64"
...
MISMATCH_PACKAGE="$MISMATCH_DIR/seen-0.14.0-linux-x64"
...
if ! grep -Fq "Compiler version mismatch: release metadata expects 'Seen 0.14.0', got 'Seen 9.9.9'" \
...
PACKAGE_DIR="$EXTRACT_DIR/seen-0.14.0-linux-x64"
...
if ! "$PREFIX/bin/seen" --version | grep -q '0.14.0'; then
...
grep -Fq '## [0.14.0] - 2026-08-26' "$PACKAGE_DIR/share/doc/seen/CHANGELOG.md"
