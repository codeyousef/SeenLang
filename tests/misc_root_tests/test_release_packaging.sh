...
#!/usr/bin/env bash
set -euo pipefail
if [[ "${1:-}" == "--expect-version" && "${2:-}" == "0.15.0" && "${3:-}" == "version" ]]; then
    echo "seen-pkg 0.15.0"
    exit 0
fi
...
GO_V4_PACKAGE="$GO_V4_DIR/seen-0.15.0-linux-x64"
...
GO_DUPLICATE_PACKAGE="$GO_DUPLICATE_DIR/seen-0.15.0-linux-x64"
...
if ! grep -Fq "Compiler version mismatch: release metadata expects 'Seen 0.15.0', got 'Seen 9.9.9'" \
...
if ! "$PREFIX/bin/seen" --version | grep -q '0.15.0'; then
...
grep -Fq '## [0.15.0] - 2026-08-26' "$PACKAGE_DIR/share/doc/seen/CHANGELOG.md"
