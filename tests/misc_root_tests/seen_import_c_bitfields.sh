...
TMP_DIR="$(mktemp -d "$SEEN_ARTIFACT_ROOT/seen-import-c-bitfields.XXXXXX")"
...
cleanup() {
    case "$TMP_DIR" in
        "$SEEN_ARTIFACT_ROOT"/seen-import-c-bitfields.*)
            [ -d "$TMP_DIR" ] && [ ! -L "$TMP_DIR" ] &&
...
bash "$ATTESTED_SEEN" "$COMPILER" compile \
    "$OUT" "$TMP_DIR/bitfields_probe" --fast >/dev/null
