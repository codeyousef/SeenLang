...
select_builder() {
    local root=$1
    shift
    bash "$SELECTOR" --repo-root "$root" --checkout-version 0.10.1 \
...
make_compiler "$scenario/compiler_seen/target/seen" current 0.10.1 "$log"
make_compiler "$scenario/target/release/seen" release 0.10.1 "$log"
make_compiler "$scenario/stage3_recovery_head" recovery 0.10.1 "$log"
make_sidecar "$scenario/source-sidecar" 0.10.1 "$log"
...
log="$scenario/invocations.log"
make_compiler "$scenario/custom/seen" explicit 0.10.1 "$log"
make_compiler "$scenario/compiler_seen/target/seen" current 0.10.1 "$log"
make_sidecar "$scenario/source-sidecar" 0.10.1 "$log"
