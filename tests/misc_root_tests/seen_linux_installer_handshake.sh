...
source_asset=seen-compiler-0.21.0-linux-x64
...
assert_handshake_invocation() {
    if ! grep -Fxq -- '--expect-version 0.21.0 version --machine' \
...
for builder_arch in \
    ...
    run_builder_failure "$builder" "$arch" \
        'does not match package version 0.21.0' \
