#!/usr/bin/env python3
"""Measure P0-GATE0-001 checker line and conditional-branch coverage."""
import ast, dis, importlib.util, sys, unittest
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
CHECKER = ROOT / "scripts/check_gate0_certification.py"
TESTS = ROOT / "tests/runner/test_gate0_certification_unit.py"
TARGETS = {"pairs", "integer", "digest", "safe_path", "validate"}
...
def main():
    tests = load("test_gate0_coverage_suite", TESTS); checker = tests.checker
    codes = {getattr(checker, name).__code__ for name in TARGETS}; lines = set(); branches = source_branches(CHECKER)
    observed_lines = set(); observed_branches = set(); previous = {}
    for code in codes:
    ...
    print(f"gate0-certification parser coverage: line={line_percent:.2f}% branch={branch_percent:.2f}% lines={len(covered_lines)}/{len(lines)} branches={len(covered_branches)}/{len(branches)}")
    if line_percent < 90 or branch_percent < 90:
    ...
    return 0 if line_percent >= 90 and branch_percent >= 90 else 1
