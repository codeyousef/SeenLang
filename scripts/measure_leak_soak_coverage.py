#!/usr/bin/env python3
"""Measure TEST-002D checker line and conditional-branch coverage."""
import ast, dis, importlib.util, sys, unittest
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
CHECKER = ROOT / "scripts/check_leak_soak_evidence.py"
TESTS = ROOT / "tests/runner/test_leak_soak_evidence_unit.py"
TARGETS = {"pairs", "integer", "validate_reading", "validate"}
...
def main():
    tests = load("test_leak_soak_coverage_suite", TESTS); checker = tests.checker
    ...
    print(f"leak-soak parser coverage: line={line_percent:.2f}% branch={branch_percent:.2f}% lines={len(covered_lines)}/{len(lines)} branches={len(covered_branches)}/{len(branches)}")
