#!/usr/bin/env python3
"""Run the host-normalized pinned seen-error-v1 microbenchmark."""
...
def load_checker(root: Path) -> object:
    spec = importlib.util.spec_from_file_location(
        "check_error_contract", root / "scripts/check_error_contract.py"
    )
    if spec is None or spec.loader is None:
        fail("could not load structured-error checker")
...
def main() -> int:
    ...
    try:
        ...
        print(f"structured-error benchmark: {error}", file=sys.stderr)
        return 1
    return 0
