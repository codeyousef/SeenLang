#!/usr/bin/env python3
"""Measure CORE-004A checker line and conditional-branch coverage."""
import ast, dis, importlib.util, sys, unittest
from pathlib import Path
ROOT = Path(__file__).resolve().parents[1]; CHECKER = ROOT / "scripts/check_bootstrap_reproducibility.py"; TESTS = ROOT / "tests/runner/test_bootstrap_reproducibility_unit.py"
TARGETS = {"pairs", "integer", "digest", "text", "validate_builder", "validate"}
def load(name, path):
...
def main():
    tests = load("test_bootstrap_reproducibility_coverage_suite", TESTS); checker = tests.checker
    ...
    covered_lines = observed_lines & lines; covered_branches = observed_branches & branches
    line_percent = 100 * len(covered_lines) / len(lines); branch_percent = 100 * len(covered_branches) / len(branches)
    print(f"bootstrap-reproducibility parser coverage: line={line_percent:.2f}% branch={branch_percent:.2f}% lines={len(covered_lines)}/{len(lines)} branches={len(covered_branches)}/{len(branches)}")
    if line_percent < 90 or branch_percent < 80:
        print("missing lines: " + ",".join(map(str, sorted(line for code, line in lines - observed_lines))), file=sys.stderr); print("missing branches: " + ",".join(f"{a}->{b}" for a, b in sorted(branches - observed_branches)), file=sys.stderr)
