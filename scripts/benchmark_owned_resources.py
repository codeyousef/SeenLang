#!/usr/bin/env python3
"""Run the pinned host-normalized P0-OWN-001 ownership benchmark."""
...
def main() -> int:
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "check_owned_resources", root / "scripts/check_owned_resources.py"
        )
        if spec is None or spec.loader is None:
            raise ValueError("could not load ownership checker")
        checker = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(checker)
        raw = args.contract.read_bytes()
        expected = checker.evaluate(checker.validate(raw))
        ...
        ratio = int(statistics.median(candidates)) * 1_000_000 // int(statistics.median(controls))
        ceiling = baseline["baseline_ratio_ppm"] * 105 // 100
        if ratio > ceiling:
            raise ValueError(f"normalized ratio {ratio} ppm exceeds 5% ceiling {ceiling} ppm")
        print(f"owned-resource benchmark: ratio_ppm={ratio} ceiling_ratio_ppm={ceiling} warmups=5 samples=30 status=pass")
    except (OSError, json.JSONDecodeError, ValueError) as error:
        print(f"owned-resource benchmark: {error}", file=sys.stderr)
        return 1
    return 0
