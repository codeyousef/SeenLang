#!/usr/bin/env python3
"""Run the host-normalized pinned seen-typed-error-v1 microbenchmark."""
...
def load_checker(root: Path) -> object:
    spec = importlib.util.spec_from_file_location(
        "check_typed_errors", root / "scripts/check_typed_errors.py"
    )
    if spec is None or spec.loader is None:
        fail("could not load typed-error checker")
...
def main() -> int:
    ...
    try:
        ...
        def candidate_sample() -> int:
            ...
            for _ in range(iterations):
                if checker.validate(raw) != expected:
                    fail("typed-error validation changed")
        ...
        print(f"typed-error benchmark: {error}", file=sys.stderr)
        return 1
    return 0
