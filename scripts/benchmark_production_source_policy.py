#!/usr/bin/env python3
"""Run the pinned host-normalized production-source policy benchmark."""
...
def main() -> int:
    ...
    try:
        ...
        if spec is None or spec.loader is None:
            raise ValueError("could not load production-source checker")
        ...
        if baseline["version"] != 1 or baseline["warmups"] != 5 or \
            ...
            raise ValueError(
                "benchmark policy must be version 1, 5 warmups, 30 samples, 5 percent")
        ...
        def candidate() -> int:
            ...
            for _ in range(iterations):
                if checker.parse_and_validate(raw) != expected:
                    raise ValueError("production-source plan changed")
        ...
        if ratio > ceiling:
            raise ValueError(
                f"normalized ratio {ratio} ppm exceeds 5% ceiling {ceiling} ppm")
        print(
            "production-source benchmark: "
        ...
        print(f"production-source benchmark: {error}", file=sys.stderr)
        return 1
    return 0
