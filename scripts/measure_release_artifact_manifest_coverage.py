#!/usr/bin/env python3
"""Measure CORE-004B checker line and conditional-branch coverage."""
import ast, dis, importlib.util, sys, unittest
from pathlib import Path
ROOT = Path(__file__).resolve().parents[1]; CHECKER = ROOT / "scripts/check_release_artifact_manifest.py"; TESTS = ROOT / "tests/runner/test_release_artifact_manifest_unit.py"
TARGETS = {"pairs", "integer", "digest", "printable", "safe_name", "semver", "parse", "validate"}
def load(name, path):
...
def main():
    tests = load("test_release_artifact_coverage_suite", TESTS); checker = tests.checker
    ...
    for code in codes:
        for item in dis.get_instructions(code):
            if item.positions and item.positions.lineno and item.positions.lineno != code.co_firstlineno:
    ...
    print(f"release-artifact parser coverage: line={line_percent:.2f}% branch={branch_percent:.2f}% lines={len(covered_lines)}/{len(lines)} branches={len(covered_branches)}/{len(branches)}")
