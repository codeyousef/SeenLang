#!/usr/bin/env python3
"""Run the host-normalized pinned package-layout v1 microbenchmark."""
...
def load_checker(root: Path) -> object:
    ...
    if spec is None or spec.loader is None:
        fail("could not load package-layout checker")
...
def main() -> int:
    ...
    try:
        ...
        expected = (
            json.dumps(checker.parse_and_validate(raw, 65_536), indent=2, sort_keys=True)
            + "\n"
        ).encode()
        ...
        for sample_index in range(30):
            if sample_index % 2 == 0:
                ...
                candidate_samples.append(candidate_sample())
                control_samples.append(control_sample())
        candidate_median = int(statistics.median(candidate_samples))
        control_median = int(statistics.median(control_samples))
        if control_median < 1:
            fail("control median is not positive")
        ratio = paired_median_ratio_ppm(candidate_samples, control_samples)
        ceiling = baseline_ratio * 105 // 100
        if ratio > ceiling:
            fail(f"normalized ratio {ratio} ppm exceeds 5% ceiling {ceiling} ppm")
        print(
            "package-layout benchmark: "
        ...
        print(f"package-layout benchmark: {error}", file=sys.stderr)
        return 1
    return 0
