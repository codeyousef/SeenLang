...
use crate::types::{Method, MethodError, MethodSignature, MethodVisibility, ReceiverType};
use seen_parser::ast::*;
...
impl MethodManager {
    ...
    pub fn register_method(&mut self, method: Method) -> Result<(), MethodError> {
        ...
        if !self.config.enable_method_overloading {
            if let Some(existing_methods) = self
                .methods
                .get(&method.signature.receiver_type.type_name())
            {
                if existing_methods
                    .iter()
                    .any(|m| m.signature.name == method.signature.name)
                {
    ...
    pub fn resolve_method(
        ...
        let compatible_methods: Vec<_> = candidates
            .into_iter()
        ...
        let visible_methods: Vec<_> = compatible_methods
            .into_iter()
...
mod tests {
    use super::*;
    use crate::types::{MethodVisibility, Parameter, ReceiverType};
