...
### Implemented in the eighteenth patch set

- Expanded `scripts/native_target_smoke.sh` so the default native matrix no longer stops at hello-world only:
   - every target still compiles `examples/hello_world/hello_english.seen`
   - `windows-x86_64` also compiles `tests/codegen/test_game_engine_features.seen`
   - `android-arm64` also compiles `tests/gpu/test_compute_basic.seen`
- Kept `summary.tsv` at one row per target for existing consumers while adding per-target `case-results.tsv` files and aggregate `build.log` output so the broader case set is inspectable without breaking `scripts/platform_matrix.sh`.
- Marked `scripts/native_target_smoke.sh` executable so it matches its own CLI usage text and can be invoked directly like the other platform scripts.
- Fixed `scripts/platform_matrix.sh` to resolve the actual timestamped smoke `build.log` path emitted by the harness instead of recording a stale non-timestamped location in the JSON reports.
- Re-ran broadened native smoke locally and confirmed:
   - `windows-x86_64` now succeeds for both the hello-world smoke source and `tests/codegen/test_game_engine_features.seen`
   - `android-arm64` now succeeds for both the hello-world smoke source and `tests/gpu/test_compute_basic.seen` on the local NDK (`r29`, API 24)
- Re-ran `scripts/platform_matrix.sh` for `windows-x86_64` and `android-arm64` against the current stage compiler and confirmed the generated JSON reports now carry the widened success notes plus valid nested smoke log paths.

### Implemented in the nineteenth patch set

- Fixed `scripts/bundle_android.sh` so Android packaging works with the current checked-in stage compiler instead of assuming the newer `build` subcommand exists:
   - probes CLI help and falls back to `compile` when necessary
   - uses the normalized `android-arm64` target by default while still allowing override through `SEEN_ANDROID_TARGET`
   - accepts either `ANDROID_NDK_HOME` or `ANDROID_NDK_ROOT`
   - creates the output directory before writing the bundle
- Hardened `scripts/bundle_android.sh` to fail closed on unsupported `ANDROID_ABI` values instead of relabeling an ARM64 build as another ABI directory.
- Updated the Android-facing example metadata and README commands in:
   - `examples/seen-ecs-min/Seen.toml`
   - `examples/seen-vulkan-min/Seen.toml`
   - `examples/seen-ecs-min/README.md`
   - `examples/seen-vulkan-min/README.md`
  so they now use `android-arm64` and stop advertising the stale `--bundle android` CLI flow that the current stage compiler does not implement.
- Validated the real Android packaging helper locally by running `scripts/bundle_android.sh` against `examples/android/hello_ndk/main.seen` on the local NDK install and confirming it now produces an `.aab` with the expected manifest, dex, and JNI library payload.

### Implemented in the twentieth patch set

- Added `--release` support to `scripts/native_target_smoke.sh` so cross-target release-mode validation can reuse the same per-target source selection, artifact checks, and reporting path as the default compile-smoke harness instead of relying on ad hoc shell commands.
- Re-ran release-mode cross-target validation on the current Linux host and confirmed:
   - `windows-x86_64` release builds succeed for both `examples/hello_world/hello_english.seen` and `tests/codegen/test_game_engine_features.seen`
   - `android-arm64` release builds succeed for both `examples/hello_world/hello_english.seen` and `tests/gpu/test_compute_basic.seen` on the local NDK (`r29`, API 24)
- Narrowed the earlier release-mode blocker from “unvalidated after the merged-llc bypass” to broader follow-through on Apple hosts, Linux ARM64 when a sysroot is available, and continued matrix expansion rather than an unknown Windows/Android release-path risk.

### Implemented in the twenty-first patch set

- Fixed the focused `HashMap` / `StringHashMap` stdlib blockers that surfaced during native-target follow-through:
   - `seen_runtime/seen_runtime.c` now returns real `Option` handles from `HashMap_get(...)` and `HashMap_get_str(...)` via `BTreeMap_make_some(...)` / `BTreeMap_make_none()`, matching the current codegen ABI and removing the Linux `hash_map_basic` post-resize crash.
   - `compiler_seen/src/codegen/ir_type_info.seen`, `compiler_seen/src/codegen/ir_type_tables.seen`, `compiler_seen/src/codegen/ir_type_dispatch.seen`, `compiler_seen/src/codegen/ir_call_dispatch.seen`, and `compiler_seen/src/codegen/llvm_ir_gen.seen` now infer and lower `StringHashMap` operations through the existing string-keyed `HashMap` runtime specializations, including `.new()`, `.withCapacity()`, `insert`, `get`, `getOrDefault`, `contains`, `remove`, `clear`, `len`, and `isEmpty`.
   - `compiler_seen/src/codegen/ir_type_dispatch.seen` now consumes the ignored `i1` result from `HashMap_insert_*` in the `StringHashMap.insert` shim, which fixes the malformed LLVM textual IR numbering failure that previously stopped `opt` on `seen_std/tests/string_hash_map_basic.seen`.
- Rebuilt the compiler with `./scripts/safe_rebuild.sh` and confirmed the current Linux host again completes `S2->S3` successfully.
- Re-ran focused stdlib validation and confirmed:
   - `seen_std/tests/hash_map_basic.seen` passes `19/19` on Linux.
   - `seen_std/tests/string_hash_map_basic.seen` now compiles, links, and exits `0` on Linux.
   - Windows compile-only smoke succeeds for both tests via `scripts/native_target_smoke.sh --target windows-x86_64 --source ...`, producing real PE32+ x86-64 artifacts on the local MinGW toolchain.

### Implemented in the twenty-second patch set

- Widened `scripts/native_target_smoke.sh` so the default `windows-x86_64` source set now includes:
   - `examples/hello_world/hello_english.seen`
   - `tests/codegen/test_game_engine_features.seen`
   - `seen_std/tests/hash_map_basic.seen`
   - `seen_std/tests/string_hash_map_basic.seen`
- Re-ran the default Windows smoke harness and confirmed all four cases now succeed in the normal multi-case path, each producing a real PE32+ x86-64 artifact on the local MinGW toolchain.
- Re-ran the widened Windows smoke harness with `--release` and confirmed the same four-case set still succeeds in release mode, extending the Windows release smoke tier beyond the earlier hello-world plus game-engine coverage.
- Re-ran `scripts/platform_matrix.sh --stage3 compiler_seen/target/seen --platform windows-x86_64` and confirmed the generated JSON report now carries the widened four-case Windows success note plus the correct nested native-smoke log path.
...
- Broaden Android validation beyond the current hello-world plus compile-only compute smoke and the validated `hello_ndk` bundle path on a real NDK installation, especially richer GPU runtime coverage, broader stdlib/runtime paths, and eventual on-device execution.
