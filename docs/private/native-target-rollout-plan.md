...
### Implemented in the twenty-third patch set

- Widened `scripts/native_target_smoke.sh` so the default `android-arm64` source set now includes:
   - `examples/hello_world/hello_english.seen`
   - `tests/gpu/test_compute_basic.seen`
   - `seen_std/tests/hash_map_basic.seen`
   - `seen_std/tests/string_hash_map_basic.seen`
- Re-ran the default Android smoke harness against the local NDK (`r29`, API 24) and confirmed all four cases succeed, each producing a real Android ARM64 shared object.
