...
- Broaden Windows validation beyond the current hello-world plus focused compile-only stdlib smoke (`hash_map_basic` and `string_hash_map_basic`) on a real MinGW/LLVM toolchain and confirm whether richer runtime/stdlib coverage needs any additional Windows system libraries.
