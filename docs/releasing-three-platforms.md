...
3. Copy the macOS arm64 archive, Windows x64 ZIP, and NSIS installer to one
   ...
   A pre-existing release is a stop condition.
4. Dispatch `.github/workflows/release.yml` **on the tag ref**, with its exact
   `version` input, once. The required `windows-smoke` job downloads and hashes
   the draft on a real Windows x64 runner and executes the compiler and package
   ...
   platforms. Only after re-downloading and byte-comparing the complete draft
