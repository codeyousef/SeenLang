...
- `#[repr(packed(N))]` packs structs to `N` alignment; accessing misaligned fields is `unsafe`.
...
- Projects using `visibility = "caps"` automatically export capitalized items; `visibility = "explicit"` requires `pub`.
...
- FFI modules must document ownership transfer: `take`/`borrow` conventions surface via attributes (`#[ffi(borrow)]`, `#[ffi(transfer)]`).
