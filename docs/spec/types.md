...
- Specs provide ad-hoc polymorphism; sealed specs restrict downstream implementations.
...
| Form | Description |
| --- | --- |
| `T` | Named types: data structs, enums, specs, aliases. |
...
- **Spec bounds**: `fun draw<T: Renderable>(x: T)`.
- **Associated types**: `spec Iterator { type Item; fun Next(self) -> Option<Self::Item>; }`.
- **Inference rules**:
  - Local type inference saturates function bodies; unspecified types default to `_`.
  - Spec resolution prefers concrete impls; conflicting impls raise deterministic diagnostics with source references.
  - Monomorphisation occurs before codegen; each instantiation is hashed for determinism.

## 4. Specs & Implementations
- Specs may include methods (with default implementations) and associated types/constants.
- **Sealed specs**: `sealed spec ShaderStage {}` restricts impls to defining crate; external impl attempts error.
- Orphan rule: spec `T` can be implemented for type `S` if either `T` or `S` is defined in current crate.
