...
impl SeenLanguageServer {
        ...
        let program = match parser.parse_program() {
            ...
            Err(e) => {
                ...
                let pos = match &e {
                    ...
                    seen_parser::error::ParseError::LexerError { .. } => &seen_lexer::Position {
                        line: 1,
                        column: 1,
                        offset: 0,
                    },
    ...
    async fn get_completions(&self, _uri: &Url, _position: Position) -> Vec<CompletionItem> {
        ...
        items.push(CompletionItem {
            ...
            insert_text: Some(
                "pure fun ${1:name}(${2:params}): ${3:ReturnType} = ${4:expression}".to_string(),
            ),
            insert_text_format: Some(InsertTextFormat::SNIPPET),
            ..Default::default()
        });
    ...
    async fn find_symbol_at_position(&self, uri: &Url, position: Position) -> Option<String> {
                ...
                for (line_idx, line) in lines.iter().enumerate() {
                    ...
                    while let Some(pos) = line[col..].find(&symbol_name) {
                        ...
                        if is_word_start && is_word_end {
                            ...
                            let is_definition = doc
                                .symbols
                                .get(&symbol_name)
                                .map_or(false, |sym| sym.definition.range == range);
            ...
            if let Some(line_content) = lines.get(range.start.line as usize) {
                    ...
                    let edit = TextEdit {
                        range: Range {
                            start: Position {
                                ...
                                character: line_content.trim_end().len() as u32,
                            },
                            end: Position {
                                line: range.start.line,
                                character: line_content.trim_end().len() as u32,
                ...
                if range.start.line != range.end.line
                    || (range.start.line == range.end.line
                        && range.end.character - range.start.character > 10)
                {
                    actions.push(CodeActionOrCommand::CodeAction(CodeAction {
                    ...
                    let edit = TextEdit {
                        range: Range {
                            start: Position {
                                ...
                                line: last_line.saturating_sub(1),
