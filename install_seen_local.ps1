# Local Seen Language Installer for Windows
# Installs Seen system-wide from the current project directory
...
Write-Host "     Seen Language Local Installer            " -ForegroundColor Cyan
Write-Host "===============================================" -ForegroundColor Cyan
Write-Host ""
...
if ($System) {
    if (-not (Test-Administrator)) {
        Write-Error-Custom "System-wide installation requires administrator privileges. Run PowerShell as Administrator."
    }
...
$compilerPath = Join-Path $projectRoot "compiler_seen\target\seen.exe"

if (-not (Test-Path $compilerPath)) {
    Write-Error-Custom "Seen compiler not found at: $compilerPath. Please build the project first."
}
...
# Copy standard library (if exists)
$stdlibPath = Join-Path $projectRoot "stdlib"
if (Test-Path $stdlibPath) {
    Write-Info "Installing standard library..."
    Copy-Item $stdlibPath (Join-Path $libDir "seen") -Recurse -Force
}
...
if (Test-Path $docsPath) {
    ...
    Copy-Item $docsPath (Join-Path $shareDir "docs") -Recurse -Force
}
...
try {
    ...
    Write-Host "Warning: Failed to create Start Menu shortcuts." -ForegroundColor Yellow
}
