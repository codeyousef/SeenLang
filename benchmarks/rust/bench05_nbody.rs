...
const N_STEPS: usize = 50_000_000;
const DT: f64 = 0.01;
...
fn advance(bodies: &mut [Body; 5]) {
    for i in 0..5 {
        for j in (i + 1)..5 {
            ...
            bodies[i].vx -= dx * mj_mag;
            bodies[i].vy -= dy * mj_mag;
            bodies[i].vz -= dz * mj_mag;

            bodies[j].vx += dx * mi_mag;
            bodies[j].vy += dy * mi_mag;
            bodies[j].vz += dz * mi_mag;
        }
    }
...
fn main() {
    ...
    let position_checksum = bodies.iter()
        .map(|b| b.x + b.y + b.z)
        .sum::<f64>();

    println!("Initial energy: {:.9}", initial_energy);
    println!("Final energy: {:.9}", final_energy);
    println!("Position checksum: {:.9}", position_checksum);
