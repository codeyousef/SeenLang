...
function Run-BenchmarkCategory {
    ...
    Write-Host "[OK] $CategoryName benchmarks completed" -ForegroundColor Green
}
...
if (Test-Command "cargo") {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "[OK] Rust benchmarks built" -ForegroundColor Green
    } else {
        Write-Host "[WARNING] Failed to build Rust benchmarks" -ForegroundColor Yellow
    }
    Pop-Location
} else {
    Write-Host "[WARNING] Rust not found, skipping Rust benchmarks" -ForegroundColor Yellow
}
...
if ($CppCompiler) {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "[OK] C++ benchmarks built" -ForegroundColor Green
    } else {
        Write-Host "[WARNING] Failed to build C++ benchmarks" -ForegroundColor Yellow
    }
} else {
    Write-Host "[WARNING] No C++ compiler found, skipping C++ benchmarks" -ForegroundColor Yellow
}
...
if (Test-Command "zig") {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "[OK] Zig benchmarks built" -ForegroundColor Green
    } else {
        Write-Host "[WARNING] Failed to build Zig benchmarks" -ForegroundColor Yellow
    }
    Pop-Location
} else {
    Write-Host "[WARNING] Zig not found, skipping Zig benchmarks" -ForegroundColor Yellow
}
...
if (Test-Path $RustBench) {
    ...
    Write-Host "[OK] Rust benchmarks completed" -ForegroundColor Green
}
...
if (Test-Path $CppBench) {
    ...
    Write-Host "[OK] C++ benchmarks completed" -ForegroundColor Green
}
...
if (Test-Path $ZigBench) {
    ...
    Write-Host "[OK] Zig benchmarks completed" -ForegroundColor Green
}
...
if ($Validate) {
    ...
    if (Select-String -Path $ValidationReport -Pattern '"validation_status": "validated"' -Quiet) {
        Write-Host "[OK] Performance claims validated!" -ForegroundColor Green
    } else {
        Write-Host "[WARNING] Some performance claims could not be validated" -ForegroundColor Yellow
...
if ($Compare) {
    ...
    if (Select-String -Path $ComparisonReport -Pattern '"regression_detected": true' -Quiet) {
        ...
        Write-Host "[OK] No performance regressions detected" -ForegroundColor Green
    }
}
