...
pub fn build_project(project_path: &Path) -> Result<()> {
    ...
    let main_file = find_main_file(project_path).context("Failed to find main source file")?;

    println!(
        "  {} Main file: {}",
        "•".yellow(),
        main_file.display().to_string().blue()
    );
...
fn compile_project(
    ...
    let keyword_config = seen_lexer::keyword_config::KeywordConfig::from_directory(&lang_dir)
        .context(format!(
            "Failed to load keyword configuration from {}",
            lang_dir.display()
        ))?;

    // Create the keyword manager with the configuration and language setting
    let keyword_manager =
        seen_lexer::keyword_config::KeywordManager::new(keyword_config, config.language.clone())
            .context("Failed to create keyword manager")?;

    // Create the lexer
    let mut lexer =
        seen_lexer::lexer::Lexer::new(&source_code, &keyword_manager, config.language.clone());

    // Tokenize the source code
    let tokens = lexer.tokenize().context("Lexical analysis failed")?;

    println!("    {} {} tokens generated", "✓".green(), tokens.len());

    // 2. Parsing
    println!("  {} Parsing...", "→".cyan());
    let ast = seen_parser::parse(tokens).context("Parsing failed")?;
    ...
    println!(
        "    {} LLVM IR generated: {}",
        "✓".green(),
        ir_file.display().to_string().blue()
    );
    ...
    println!(
        ...
        "✓".green(),
        exe_file.display().to_string().blue()
    );
