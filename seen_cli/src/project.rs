...
pub fn create_new_project(name: &str) -> Result<()> {
    ...
    create_main_file(&src_dir, &config.language).context("Failed to create main.seen file")?;

    // Create README.md file
    create_readme_file(&project_dir, name).context("Failed to create README.md file")?;

    println!(
        "{} Created new Seen project: {}",
        "✓".green().bold(),
        name.blue().bold()
    );
...
"#
        ...
        "arabic" => {
            r#"// برنامج مرحبا بالعالم في لغة سين
