...
func TestVersionHandshake(t *testing.T) {
	...
	code := Run(context.Background(), []string{"--expect-version", "0.18.1", "version", "--machine"}, &out, &errOut)
...
func TestGlobalCLIParsing(t *testing.T) {
	...
	tests := []struct {
		...
		{"identical repeat", []string{"--expect-version=0.18.1", "--expect-version", "0.18.1", "version"}, 0, "seen-pkg", ""},
		{"unknown", []string{"--unknown"}, 64, "", "unknown global option"},
		{"missing value", []string{"--expect-version"}, 64, "", "requires a value"},
		{"conflicting repeat", []string{"--expect-version", "0.18.1", "--expect-version", "0.10.2", "version"}, 64, "", "conflicting values"},
