...
- Added native `seen-build-instrumentation-evidence-v1` policy and evidence,
  ...
  modules, the retained C runtime, and ledgered ABI shims; compiler builds emit
  compile-only evidence and cannot self-certify hardware execution.
...
- Added native deterministic global-initialization planning with a checked
  ...
  end-to-end cross-module runtime regression shared by compile, check, and JIT.
  Per-module LLVM constructors now receive unique bounded priorities, and the
  object-cache compatibility identity is `seen-object-cache-abi-v3`.
...
- Bumped the shipped Seen compiler, workspace, CLI, LSP, version-coupled package client, release documentation and tests, and benchmark baseline metadata to `0.10.1`.

### Fixed

- Fixed generic `Result` pointer-payload lowering so erased integer handles are reified to pointers before use, preventing invalid LLVM IR and compiler crashes during full project builds.
...
- Changed Vulkan RGBA8 image readback to the caller-provided `vk_read_image_rgba8_into` buffer API, avoiding unsupported generic byte-array object emission while retaining bounds checks and canonical RGBA output.
- Bumped the shipped Seen compiler metadata to `0.9.4` and `seen_std` to `0.1.2`.

### Fixed

- Fixed full native object emission for programs that import the Vulkan readback wrapper.
...
- Source `HashMap` fallback buckets now use contiguous key/value/hash/state tables instead of `Vec<Option<Entry>>` wrapper buckets, with tombstone-aware O(1) probing, and runtime-backed map imports no longer emit duplicate source-method symbols.
...
- Reworked compiler LLVM string utility cleanup and escaping helpers to use segment-based `StringBuilder` output instead of repeated substring concatenation.
