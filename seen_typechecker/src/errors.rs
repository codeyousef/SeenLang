...
pub enum TypeError {
    ...
    #[error(
        "Function '{name}' expects {expected} arguments, but {actual} were provided at {position}"
    )]
    ...
    #[error(
        "Invalid operation '{operation}' for types {left_type} and {right_type} at {position}"
    )]
    InvalidOperation {
        operation: String,
        left_type: Type,
    ...
    ImmutableAssignment { name: String, position: Position },
    ...
    IndexOutOfBounds { position: Position },
    ...
    InferenceFailed { position: Position },

    #[error("Circular type dependency detected at {position}")]
    CircularDependency { position: Position },

    #[error("Type '{name}' is already defined at {position}")]
    DuplicateType { name: String, position: Position },
...
impl TypeError {
    ...
    pub fn kind(&self) -> TypeErrorKind {
        match self {
            ...
            | TypeError::UnknownField { .. } => TypeErrorKind::UndefinedReference,
