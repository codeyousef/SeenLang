...
fn test_smart_casting_condition_analysis() {
    ...
    checker.env.define_variable(
        "user".to_string(),
        seen_typechecker::Type::Nullable(Box::new(user_struct.clone())),
    );
...
fn test_environment_smart_cast_methods() {
    ...
    checker
        .env
        .add_smart_cast("test_var".to_string(), seen_typechecker::Type::String);
