...
use crate::types::{AsyncError, AsyncResult, AsyncValue, TaskId};
...
impl AsyncFunction {
    ...
    fn execute_expression<'a>(
        ...
        Box::pin(async move { self.execute_expression_inner(expr, context).await })
    }
        ...
        match expr {
            ...
            Expression::Return { value, .. } => {
                if let Some(return_expr) = value {
                    let return_val =
                        Box::pin(self.execute_expression(return_expr, context)).await?;
            ...
            Expression::StructLiteral { name, fields, .. } => {
                ...
                for (_field_name, field_expr) in fields {
                    field_values
                        .push(Box::pin(self.execute_expression(field_expr, context)).await?);
                }
...
impl AsyncFunctionRegistry {
    ...
    pub fn get_public_functions(&self) -> Vec<&AsyncFunction> {
        self.functions.values().filter(|f| f.is_public).collect()
    }
...
mod tests {
    ...
    fn test_async_function_signature() {
        ...
        assert_eq!(
            signature,
            "async fun processData(input: String, config: Config?): Result"
        );
