...
"#
    ...
    fn test_load_valid_toml_file() {
        ...
        assert!(
            result.is_ok(),
            "Failed to load valid TOML file: {:?}",
            result.err()
        );
        ...
        assert_eq!(
            english.description,
            "English keyword set for the Seen programming language"
        );
        ...
        assert_eq!(
            english.keyword_map.get(&fun_keyword),
            Some(&KeywordType::KeywordFun)
        );
        assert_eq!(
            english.keyword_map.get(&if_keyword),
            Some(&KeywordType::KeywordIf)
        );
        assert_eq!(
            english.keyword_map.get(&and_keyword),
            Some(&KeywordType::KeywordAnd)
        );

        // Test reverse mapping
        assert_eq!(
            english.reverse_map.get(&KeywordType::KeywordFun),
            Some(&fun_keyword)
        );
        assert_eq!(
            english.reverse_map.get(&KeywordType::KeywordIf),
            Some(&if_keyword)
        );
    }
    ...
    fn test_load_multiple_languages() {
...

        {
            ...
            assert_eq!(
                arabic.keyword_map.get(&ar_fun_keyword),
                Some(&KeywordType::KeywordFun)
            );
            assert_eq!(
                arabic.keyword_map.get(&ar_if_keyword),
                Some(&KeywordType::KeywordIf)
            );
            assert_eq!(
                arabic.keyword_map.get(&ar_and_keyword),
                Some(&KeywordType::KeywordAnd)
            );
        }
    }
    ...
    fn test_is_keyword_functionality() {
        ...
        assert_eq!(
            manager.is_keyword(&fun_keyword),
            Some(KeywordType::KeywordFun)
        );
        assert_eq!(
            manager.is_keyword(&if_keyword),
            Some(KeywordType::KeywordIf)
        );
        assert_eq!(
        ...
        assert_eq!(
            manager.is_keyword(&or_keyword),
            Some(KeywordType::KeywordOr)
        );
        assert_eq!(
            manager.is_keyword(&not_keyword),
            Some(KeywordType::KeywordNot)
        );
    ...
    fn test_language_switching() {
        ...
        assert_eq!(
            manager.is_keyword(&en_fun_keyword),
            Some(KeywordType::KeywordFun)
        );
        ...
        assert_eq!(
            manager.is_keyword(&ar_fun_keyword_dynamic),
            Some(KeywordType::KeywordFun)
        );
    ...
    fn test_thread_safety() {
        ...
        for i in 0..10 {
            ...
            let handle = thread::spawn(move || {
                for _ in 0..100 {
                    let fun_keyword = manager_clone
                        .get_keyword_text(&KeywordType::KeywordFun)
                        .unwrap();
                    let and_keyword = manager_clone.get_logical_and();

                    assert_eq!(
                        manager_clone.is_keyword(&fun_keyword),
                        Some(KeywordType::KeywordFun)
                    );
                    assert_eq!(
                        manager_clone.is_keyword(&and_keyword),
                        Some(KeywordType::KeywordAnd)
                    );
                }
                i
    ...
    fn test_validate_all_languages() {
        ...
        let en_path = create_test_toml_file(&temp_dir, "en.toml", create_valid_english_toml());
        let incomplete_path =
            create_test_toml_file(&temp_dir, "incomplete.toml", create_incomplete_toml());

        let mut manager = KeywordManager::new();
        manager.load_from_toml_file(&en_path, "en").unwrap();
        manager
            .load_from_toml_file(&incomplete_path, "incomplete")
            .unwrap();

        let result = manager.validate_all_languages();
        assert!(
            result.is_err(),
            "Should fail validation due to incomplete language"
        );
    }
    ...
    fn test_load_minimum_ten_languages() {
        ...
        assert_eq!(
            loaded_languages.len(),
            10,
            "Should have loaded exactly 10 languages"
        );

        for lang in &languages {
            assert!(
                loaded_languages.contains(&lang.to_string()),
                "Missing language: {}",
                lang
            );
        }
...
"#;
        ...
        manager.load_from_toml_file(&en_path, "en").unwrap();
        manager
            .load_from_toml_file(&es_path, "es_incomplete")
            .unwrap();
...
"#;
        ...
        assert_eq!(
            manager.is_keyword("функция"),
            Some(KeywordType::KeywordElse)
        );
    ...
    fn test_comprehensive_language_validation() {
        ...
        assert!(
            result.is_ok(),
            "All languages should pass validation: {:?}",
            result.err()
        );
        ...
        for lang in &languages {
            ...
            assert!(
                manager.is_keyword(&manager.get_logical_and()).is_some(),
                "Language {} missing logical AND",
                lang
            );
            assert!(
                manager.is_keyword(&manager.get_logical_or()).is_some(),
                "Language {} missing logical OR",
                lang
            );
            assert!(
                manager.is_keyword(&manager.get_logical_not()).is_some(),
                "Language {} missing logical NOT",
                lang
            );
        }
    }
    ...
    fn test_keyword_validation_across_all_languages() {
        ...
        for (lang_name, lang_keywords) in languages_map.iter() {
            ...
            if !missing.is_empty() {
                panic!(
                    "Language '{}' is missing keyword types: {:?}",
                    lang_name, missing
                );
            }
            ...
            if !extra.is_empty() {
                panic!(
                    "Language '{}' has extra keyword types: {:?}",
                    lang_name, extra
                );
            }
        }
...
"#;
        ...
        assert_eq!(
            manager.is_keyword(&es_fun_keyword),
            Some(KeywordType::KeywordFun)
        );
        assert_eq!(
            manager.is_keyword(&es_if_keyword),
            Some(KeywordType::KeywordIf)
        );
        assert_eq!(
            manager.is_keyword(&es_and_keyword),
            Some(KeywordType::KeywordAnd)
        );
        assert_eq!(
            manager.is_keyword(&es_or_keyword),
            Some(KeywordType::KeywordOr)
        );
        ...
        let en_borrow_keyword = manager
            .get_keyword_text(&KeywordType::KeywordBorrow)
            .unwrap();
        let en_inout_keyword = manager
            .get_keyword_text(&KeywordType::KeywordInout)
            .unwrap();
        let en_is_keyword = manager.get_keyword_text(&KeywordType::KeywordIs).unwrap();
        let en_while_keyword = manager
            .get_keyword_text(&KeywordType::KeywordWhile)
            .unwrap();
        ...
        assert_eq!(
            manager.is_keyword_with_fallback(&en_else_keyword),
            Some(KeywordType::KeywordElse)
        );
        assert_eq!(
            manager.is_keyword_with_fallback(&en_not_keyword),
            Some(KeywordType::KeywordNot)
        );
        assert_eq!(
            manager.is_keyword_with_fallback(&en_move_keyword),
            Some(KeywordType::KeywordMove)
        );
        assert_eq!(
            manager.is_keyword_with_fallback(&en_borrow_keyword),
            Some(KeywordType::KeywordBorrow)
        );
        assert_eq!(
            manager.is_keyword_with_fallback(&en_inout_keyword),
            Some(KeywordType::KeywordInout)
        );
        assert_eq!(
            manager.is_keyword_with_fallback(&en_is_keyword),
            Some(KeywordType::KeywordIs)
        );
        assert_eq!(
            manager.is_keyword_with_fallback(&en_while_keyword),
            Some(KeywordType::KeywordWhile)
        );
        assert_eq!(
            manager.is_keyword_with_fallback(&en_for_keyword),
            Some(KeywordType::KeywordFor)
        );
        ...
        assert_eq!(
            manager.get_keyword_text(&KeywordType::KeywordFun),
            Some(es_fun_keyword)
        );
        assert_eq!(
            manager.get_keyword_text(&KeywordType::KeywordIf),
            Some(es_if_keyword)
        );
        assert_eq!(
            manager.get_keyword_text(&KeywordType::KeywordElse),
            Some(en_else_keyword)
        ); // Falls back to English
        assert_eq!(
            manager.get_keyword_text(&KeywordType::KeywordNot),
            Some(en_not_keyword)
        ); // Falls back to English
    }
    ...
    fn test_comprehensive_multi_language_performance() {
        ...
        assert!(
            ...
            "Multi-language performance too slow: {:?}",
            duration
        );
    }
    ...
    fn test_fallback_language_management() {
        ...
        assert_eq!(
            manager.is_keyword_with_fallback("دالة"),
            Some(KeywordType::KeywordFun)
        );
        assert_eq!(
            manager.is_keyword_with_fallback("إذا"),
            Some(KeywordType::KeywordIf)
        );
    }
    ...
    fn test_all_ten_languages_keyword_consistency() {
        ...
        for lang in &languages {
            ...
            assert!(
                !and_keyword.is_empty(),
                "Language {} has empty AND keyword",
                lang
            );
            assert!(
                !or_keyword.is_empty(),
                "Language {} has empty OR keyword",
                lang
            );
            assert!(
                !not_keyword.is_empty(),
                "Language {} has empty NOT keyword",
                lang
            );

            // Keywords should be recognized
            assert!(
                manager.is_keyword(&and_keyword).is_some(),
                "Language {} AND keyword '{}' not recognized",
                lang,
                and_keyword
            );
            assert!(
                manager.is_keyword(&or_keyword).is_some(),
                "Language {} OR keyword '{}' not recognized",
                lang,
                or_keyword
            );
            assert!(
                manager.is_keyword(&not_keyword).is_some(),
                "Language {} NOT keyword '{}' not recognized",
                lang,
                not_keyword
            );
    ...
    fn test_language_switching_performance_stress() {
        ...
        assert!(
            duration.as_millis() < 100,
            "Language switching performance too slow: {:?}",
            duration
        );
    }
}
