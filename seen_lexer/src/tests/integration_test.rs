...
"#;
    ...
    assert!(tokens
        .iter()
        .any(|t| t.token_type == TokenType::StringLiteral && t.lexeme.contains("Hello, World!")));
}
...
"#;
    ...
    assert!(tokens
        .iter()
        .any(|t| t.token_type == TokenType::StringLiteral && t.lexeme.contains("مرحباً، يا عالم!")));
}
...
fn test_language_consistency() {
    ...
    let english_types: Vec<_> = english_tokens
        .iter()
        .map(|t| t.token_type.clone())
        .collect();
    let arabic_types: Vec<_> = arabic_tokens.iter().map(|t| t.token_type.clone()).collect();
...
fn test_source_location_tracking() {
    ...
    let func_token = tokens
        .iter()
        .find(|t| t.token_type == TokenType::Func)
        .unwrap();
    ...
    let val_token = tokens
        .iter()
        .find(|t| t.token_type == TokenType::Val)
        .unwrap();
