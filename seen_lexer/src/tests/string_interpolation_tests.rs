    ...
    fn test_simple_interpolation() {
        ...
        match token.token_type {
            TokenType::InterpolatedString(parts) => {
                ...
                assert_eq!(
                    parts[0].kind,
                    InterpolationKind::Text("Hello, ".to_string())
                );

                // Second part: {name}
                assert_eq!(
                    parts[1].kind,
                    InterpolationKind::Expression("name".to_string())
                );
    ...
    fn test_multiple_interpolations() {
        ...
        match token.token_type {
            TokenType::InterpolatedString(parts) => {
                ...
                assert_eq!(
                    parts[1].kind,
                    InterpolationKind::Expression("firstName".to_string())
                );
                assert_eq!(parts[2].kind, InterpolationKind::Text(" ".to_string()));
                assert_eq!(
                    parts[3].kind,
                    InterpolationKind::Expression("lastName".to_string())
                );
                assert_eq!(parts[4].kind, InterpolationKind::Text(" is ".to_string()));
                assert_eq!(
                    parts[5].kind,
                    InterpolationKind::Expression("age".to_string())
                );
                assert_eq!(
                    parts[6].kind,
                    InterpolationKind::Text(" years old".to_string())
                );
            }
            _ => panic!("Expected InterpolatedString token"),
        }
    ...
    fn test_complex_expression_interpolation() {
        ...
        match token.token_type {
            TokenType::InterpolatedString(parts) => {
                ...
                assert_eq!(
                    parts[0].kind,
                    InterpolationKind::Text("Result: ".to_string())
                );
                assert_eq!(
                    parts[1].kind,
                    InterpolationKind::Expression("a + b * 2".to_string())
                );
            }
            _ => panic!("Expected InterpolatedString token"),
        }
    ...
    fn test_method_call_interpolation() {
        ...
        match token.token_type {
            TokenType::InterpolatedString(parts) => {
                ...
                assert_eq!(
                    parts[1].kind,
                    InterpolationKind::Expression("user.getName().toUpperCase()".to_string())
                );
            }
            _ => panic!("Expected InterpolatedString token"),
        }
    ...
    fn test_escaped_brace_with_interpolation() {
        ...
        match token.token_type {
            TokenType::InterpolatedString(parts) => {
                ...
                assert_eq!(
                    parts[0].kind,
                    InterpolationKind::Text("{literal} and ".to_string())
                );
                assert_eq!(
                    parts[1].kind,
                    InterpolationKind::Expression("interpolated".to_string())
                );
            }
...
mod nested_brace_tests {
    ...
    fn test_nested_object_literal() {
        ...
        match token.token_type {
            TokenType::InterpolatedString(parts) => {
                ...
                assert_eq!(
                    parts[1].kind,
                    InterpolationKind::Expression(r#"User { name: "Alice", age: 30 }"#.to_string())
                );
            }
    ...
    fn test_nested_lambda() {
        ...
        match token.token_type {
            TokenType::InterpolatedString(parts) => {
                ...
                assert_eq!(
                    parts[1].kind,
                    InterpolationKind::Expression("list.map({ x -> x * 2 })".to_string())
                );
            }
    ...
    fn test_deeply_nested_braces() {
        let mut lexer =
            create_test_lexer(r#""Complex: {fn() { if (true) { return { value: 42 } } }}""#);
        ...
        match token.token_type {
            TokenType::InterpolatedString(parts) => {
                ...
                assert_eq!(
                    parts[0].kind,
                    InterpolationKind::Text("Complex: ".to_string())
                );
                assert_eq!(
                    parts[1].kind,
                    InterpolationKind::Expression(
                        "fn() { if (true) { return { value: 42 } } }".to_string()
                    )
                );
            }
    ...
    fn test_interpolation_with_all_features() {
        ...
        match token.token_type {
            TokenType::InterpolatedString(parts) => {
                ...
                assert_eq!(
                    parts[1].kind,
                    InterpolationKind::Expression("user.name".to_string())
                );
                assert_eq!(
                    parts[2].kind,
                    InterpolationKind::Text(", Age: ".to_string())
                );
                assert_eq!(
                    parts[3].kind,
                    InterpolationKind::Expression("user.age".to_string())
                );
                assert_eq!(
                    parts[4].kind,
                    InterpolationKind::Text(", Status: {active}, Lambda: ".to_string())
                );
                assert_eq!(
                    parts[5].kind,
                    InterpolationKind::Expression("list.map({ x -> x * 2 })".to_string())
    ...
    fn test_interpolation_at_string_boundaries() {
        ...
        match token.token_type {
            TokenType::InterpolatedString(parts) => {
                ...
                assert_eq!(
                    parts[1].kind,
                    InterpolationKind::Text(" at start".to_string())
                );
            }
            _ => panic!("Expected InterpolatedString"),
        }
        ...
        match token.token_type {
            TokenType::InterpolatedString(parts) => {
                ...
                assert_eq!(
                    parts[0].kind,
                    InterpolationKind::Text("at end ".to_string())
                );
                assert_eq!(
                    parts[1].kind,
                    InterpolationKind::Expression("name".to_string())
        ...
        match token.token_type {
            TokenType::InterpolatedString(parts) => {
                ...
                assert_eq!(
                    parts[0].kind,
                    InterpolationKind::Expression("onlyInterpolation".to_string())
                );
            }
            _ => panic!("Expected InterpolatedString"),
        }
