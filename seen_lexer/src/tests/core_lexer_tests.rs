...
use crate::{
    keyword_manager::KeywordType, KeywordManager, Lexer, LexerConfig, LexerError, Position,
    TokenType, VisibilityPolicy,
};
use std::sync::Arc;
...
fn create_test_lexer_with_config(input: &str, config: LexerConfig) -> Lexer {
    let mut keyword_manager = KeywordManager::new();
    keyword_manager.load_from_toml("en").unwrap();
    keyword_manager.switch_language("en").unwrap();

    Lexer::with_config(input.to_string(), Arc::new(keyword_manager), config)
}
...
mod basic_tokenization_tests {
    ...
    fn test_tokenize_string_literals() {
        ...
        assert_eq!(
            token3.token_type,
            TokenType::StringLiteral("with spaces".to_string())
        );
    ...
    fn test_tokenize_identifiers_with_visibility() {
        ...
        assert_eq!(
            token1.token_type,
            TokenType::PublicIdentifier("PublicName".to_string())
        );
        ...
        assert_eq!(
            token2.token_type,
            TokenType::PrivateIdentifier("privateName".to_string())
        );
        ...
        assert_eq!(
            token3.token_type,
            TokenType::PrivateIdentifier("_private".to_string())
        );
        ...
        assert_eq!(
            token4.token_type,
            TokenType::PublicIdentifier("CONSTANT".to_string())
        );
        assert_eq!(token4.lexeme, "CONSTANT");
    }

    #[test]
    fn test_identifier_unicode_normalization() {
        let mut lexer = create_test_lexer("Cafe\u{0301}");
        let token = lexer.next_token().unwrap();
        assert_eq!(
            token.token_type,
            TokenType::PublicIdentifier("Café".to_string())
        );
        assert_eq!(token.lexeme, "Café");
    }

    #[test]
    fn test_string_literal_unicode_normalization() {
        let mut lexer = create_test_lexer("\"Cafe\u{0301}\"");
        let token = lexer.next_token().unwrap();
        assert_eq!(
            token.token_type,
            TokenType::StringLiteral("Café".to_string())
        );
        assert_eq!(token.lexeme, "\"Cafe\u{0301}\"");
    }

    #[test]
    fn test_explicit_visibility_defaults_private() {
        let config = LexerConfig {
            visibility_policy: VisibilityPolicy::Explicit,
        };
        let mut lexer = create_test_lexer_with_config("PublicName", config);
        let token = lexer.next_token().unwrap();
        assert_eq!(
            token.token_type,
            TokenType::PrivateIdentifier("PublicName".to_string())
        );
    }
    ...
    fn test_tokenize_newlines() {
        ...
        assert_eq!(
            token1.token_type,
            TokenType::PrivateIdentifier("line1".to_string())
        );
        ...
        assert_eq!(
            token3.token_type,
            TokenType::PrivateIdentifier("line2".to_string())
        );
        ...
        assert_eq!(
            token6.token_type,
            TokenType::PrivateIdentifier("line4".to_string())
        );
...
mod unicode_handling_tests {
    ...
    fn test_unicode_identifiers() {
        ...
        assert_eq!(
            token1.token_type,
            TokenType::PrivateIdentifier("变量名".to_string())
        );
        ...
        assert_eq!(
            token2.token_type,
            TokenType::PublicIdentifier("Переменная".to_string())
        );
        ...
        assert_eq!(
            token3.token_type,
            TokenType::PrivateIdentifier("مُتَغَيِّر".to_string())
        );
    ...
    fn test_unicode_string_literals() {
        ...
        assert_eq!(
            token2.token_type,
            TokenType::StringLiteral("مرحبا بالعالم".to_string())
        );

        let token3 = lexer.next_token().unwrap();
        assert_eq!(
            token3.token_type,
            TokenType::StringLiteral("🚀 Rocket".to_string())
        );
    }
    ...
    fn test_unicode_escape_sequences() {
        ...
        assert_eq!(
            token2.token_type,
            TokenType::StringLiteral("世".to_string())
        );
    ...
    fn test_unicode_position_tracking() {
        ...
        assert_eq!(
            token1.token_type,
            TokenType::PrivateIdentifier("🚀".to_string())
        );
        assert_eq!(token1.position.line, 1);
        assert_eq!(token1.position.column, 1);
        ...
        assert_eq!(
            token3.token_type,
            TokenType::PrivateIdentifier("世界".to_string())
        );
        assert_eq!(token3.position.line, 2);
...
mod dynamic_keyword_integration_tests {
    ...
    fn test_keyword_manager_integration() {
        ...
        let fun_keyword = keyword_manager
            .get_keyword_text(&crate::keyword_manager::KeywordType::KeywordFun)
            .unwrap();
        assert_eq!(
...
mod whitespace_handling_tests {
    ...
    fn test_skip_whitespace() {
        ...
        assert_eq!(
            token2.token_type,
            TokenType::PrivateIdentifier("main".to_string())
        );
