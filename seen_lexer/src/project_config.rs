...
impl ProjectConfig {
    ...
    pub fn from_file<P: AsRef<std::path::Path>>(path: P) -> Result<Self, ProjectConfigError> {
        ...
        if config.project.name.is_empty() {
            return Err(ProjectConfigError::MissingSetting(
...
pub fn create_default_project_config<P: AsRef<std::path::Path>>(
    path: P,
    project_name: &str,
    ...
    let toml_string =
        toml::to_string_pretty(&config).map_err(|e| ProjectConfigError::TomlSerError(e))?;
...
mod tests {
    ...
    fn test_create_default_project_config_success() {
        ...
        assert!(
            result.is_ok(),
            "Failed to create default project config: {:?}",
            result.err()
        );

        // Verify file content
        let content =
            fs::read_to_string(test_file_path).expect("Failed to read created config file");
        let config: ProjectConfig =
            toml::from_str(&content).expect("Failed to parse created config file");
        ...
        assert_eq!(
            ...
            Some("A Seen language project".to_string())
        );
        assert_eq!(
