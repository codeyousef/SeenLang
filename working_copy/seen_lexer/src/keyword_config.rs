...
impl KeywordConfig {
    ...
    pub fn from_directory<P: AsRef<Path>>(directory: P) -> Result<Self, KeywordConfigError> {
        ...
        if english_path.exists() {
            ...
            languages.insert(english_keywords.language.code.clone(), english_keywords.language.clone());

            // Add keywords to the mappings
            for (token_name, keyword) in english_keywords.keywords {
                let lang_map = keyword_mappings.entry(token_name).or_insert_with(HashMap::new);
        ...
        if arabic_path.exists() {
            ...
            languages.insert(arabic_keywords.language.code.clone(), arabic_keywords.language.clone());
