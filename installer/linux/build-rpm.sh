...
Arguments:
  version              Version number (e.g., 0.20.14)
...
# RPM doesn't allow hyphens in version — replace with tilde for pre-release
RPM_VERSION="${VERSION//-/\~}"

header "Building Seen Language $RPM_VERSION RPM for $ARCH"

info "Configuration:"
info "  Version: $RPM_VERSION (from $VERSION)"
...
%{_bindir}/seen
%{_bindir}/seen-pkg
%{_bindir}/compatibility-manifest.json
%{_libdir}/seen/
%{_datadir}/seen/
%{_mandir}/man1/seen.1*
...
create_source_tarball() {
    ...
    local source_name="seen-lang-$RPM_VERSION"
