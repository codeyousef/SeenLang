...
0.20.10. It is maintainer tooling, not proof that every package is already
...
checksums with hashes from the exact 0.20.10 release assets before publishing.
...
Create `Seen.toml` and `src/main.seen` yourself; 0.20.10 does not ship `init`,
...
```bash
installer/linux/build-deb.sh 0.20.10 amd64
installer/linux/build-rpm.sh 0.20.10 x86_64
installer/linux/build-appimage.sh 0.20.10 x86_64
installer/homebrew/generate-formula.sh --version 0.20.10
```

```powershell
installer\windows\build.bat 0.20.10 x64
installer\scoop\generate-manifest.ps1 -Version 0.20.10
```
