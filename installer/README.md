...
0.17.0. It is maintainer tooling, not proof that every package is already
...
checksums with hashes from the exact 0.17.0 release assets before publishing.
...
Create `Seen.toml` and `src/main.seen` yourself; 0.17.0 does not ship `init`,
...
```bash
installer/linux/build-deb.sh 0.17.0 amd64
installer/linux/build-rpm.sh 0.17.0 x86_64
installer/linux/build-appimage.sh 0.17.0 x86_64
installer/homebrew/generate-formula.sh --version 0.17.0
```

```powershell
installer\windows\build.bat 0.17.0 x64
installer\scoop\generate-manifest.ps1 -Version 0.17.0
```
