...
- `compiler_seen/src/tools/c_import_gen.seen` now collapses imported C bitfields into ABI-correct backing storage fields and emits generated getter/setter helpers for the validated <=32-bit storage-unit cases used by the current import-c regressions.
...
3. If wider 64-bit C bitfield storage units become necessary for package bindings, extend the generated helper path beyond the current validated <=32-bit storage-unit coverage.
