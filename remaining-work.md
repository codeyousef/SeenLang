...
- `compiler_seen/src/tools/c_import_gen.seen` now parses named `RecordDecl` definitions into `@repr(C)` classes, skips same-name struct typedef aliases, and emits typed record pointers in generated function signatures.
...
- `tests/misc_root_tests/seen_import_c_typedefs.sh` now covers synthetic typedef aliases plus generated `@repr(C)` record layouts, and `tests/misc_root_tests/seen_import_c_real_headers.sh` covers real `string.h`, `stdio.h`, and `vulkan.h` imports to reject bogus `prev` / `referenced` bindings while preserving Vulkan typedefs and records like `VkInstanceCreateInfo`.
...
1. Extend `import-c` beyond named struct layouts so first-party SDL/Vulkan packages can handle unions, inline arrays, and other record shapes that still collapse to coarse pointer/scalar fallbacks.
