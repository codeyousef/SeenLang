...
class SeenDebugHoverProvider implements vscode.HoverProvider {
    async provideHover(
        ...
        // Query the debug adapter for the variable value
        try {
            // Get the current stack frame
            const frameId = (activeSession as any).activeStackItem?.id;
            if (frameId) {
                ...
                if (result && result.result) {
                    ...
                    md.appendCodeblock(`${word}${typeStr} = ${result.result}`, 'seen');
                    return new vscode.Hover(md, range);
                }
            }
        } catch {
            // Evaluation failed - variable may not be in scope
        }

        // Fallback: show the variable name
        return new vscode.Hover(
            new vscode.MarkdownString(`**Debug**: \`${word}\` (not available in current scope)`),
            range
        );
    }
}
