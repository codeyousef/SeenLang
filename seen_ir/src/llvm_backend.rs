...
use crate::llvm::instructions::{normalize_method_name, get_result_method_alias, BinaryOps, ControlFlowOps, MemoryOps, AggregateOps};
    ...
    pub(crate) fn cast_basic_to_type(
        ...
        match (value, target) {
            ...
            (BasicValueEnum::StructValue(sv), BasicTypeEnum::IntType(it)) => {
                ...
                if sv.get_type() == self.ty_string() {
                    ...
                    let str_ptr = self
                        ...
                        .build_pointer_cast(
                            raw_ptr,
                            self.ty_string().ptr_type(AddressSpace::from(0u16)),
                            "string_autobox_cast",
                        )?;
            ...
            (BasicValueEnum::PointerValue(pv), BasicTypeEnum::StructType(st)) => {
                ...
                let target_ptr = self
                    .builder
                    .build_pointer_cast(pv, st.ptr_type(AddressSpace::from(0u16)), "ptrcast_struct")
                    .map_err(|e| anyhow!("{e:?}"))?;
                let loaded = self
                    .builder
